...
After import, open the playlist in MVBar and select **Manage** to configure optional Deezer synchronization. Sync can run every 6 hours, 12 hours, daily, every 3 days, or weekly, and there is also a **Sync now** action. A sync preserves tracks that still exist upstream, follows Deezer's track order, automatically downloads newly added songs, and removes songs that Deezer removed **only from the MVBar playlist**. The underlying audio files remain in the music library.
